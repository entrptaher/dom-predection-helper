export default class DomPredictionHelper {
    ...
    escapeCssNames(name) {
      if (name) {
        try {
          return name.replace(/\bselectorgadget_\w+\b/g, '').replace(/\\/g, '\\\\').replace(/[\#\;\&\,\.\+\*\~\'\:\"\!\^\$\[\]\(\)\=\>\|\/]/g, e => `\\${e}`).replace(/\s+/, '');
        } catch (e) {
          if (window.console) {
            console.log('---');
    ...
    siblingsWithoutTextNodes(e) {
      let filtered_nodes, node, nodes, _i, _len;
    ...
    cssDescriptor(node) {
      let cssName, escaped, path, _i, _len, _ref;
    ...
    cssDiff(array) {
      let collective_common, cssElem, diff, dmp, encoded_css_array, existing_tokens, part, _i, _j, _len, _len1;
      try {
    ...
    tokenizeCssForDiff(css_string) {
      let block, combined_tokens, token, _i, _len, _ref;
      combined_tokens = [];
      block = [];
    ...
    encodeCssForDiff(strings, existing_tokens) {
      let codepoint, out, string, strings_out, token, _i, _j, _len, _len1, _ref;
      codepoint = 50;
      strings_out = [];
    ...
    tokenPriorities(tokens) {
      let epsilon, first, i, priorities, second, token, _i, _len;
      epsilon = 0.001;
    ...
    orderFromPriorities(priorities) {
      let i, ordering, tmp, _i, _j, _ref, _ref1;
      tmp = new Array();
      ordering = new Array();
    ...
    wouldLeaveFreeFloatingNthChild(parts, part) {
      let i, nth_child_is_on_right, space_is_on_left;
    ...
    cleanCss(css) {
      let cleaned_css, last_cleaned_css;
      cleaned_css = css;
